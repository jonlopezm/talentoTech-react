...
import { useGetUsersQuery , useDeleteUserMutation } from '../../features/api/apiSlice';
...
export default function UserList() {
    ...
    const handleDelete = (user) => {
        Swal.fire({
            title: `Are you sure you want to delete this user${user.name} ${user.lastname}?`,
            icon: 'warning',
                ...
                {users.map(user => (
                    <tr key={user.id}>
                        ...
                        <td className="border border-slate-300 whitespace-nowrap px-1 py-1 font-medium text-black-800">
                            ...
                            src={`http://localhost:3010/${user.avatar}`} alt="" />
                        </td>
