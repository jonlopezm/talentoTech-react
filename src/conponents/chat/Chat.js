    ...
    return (
        ...
        <div className="flex flex-col  space-y-2 bg-slate-50">
            ...
            className={
                ...
                >
                <p>{message.body}</p>
                <span className="text-xs text-gay-100 self-end">{message.createdAt}</span>
            </div>
            ))}             
        </div>
        <div ref={endReference}/>
        <hr />
        <form onSubmit={handleSubmit} className="flex bg-gray-100 p-4">
            <input type="text" 
            name ="message"
            className="w-full border rounded-lg p-2" 
            placeholder="Escribe un mensaje"/>
            <button className="bg-blue-500 mx-2 text-white p-2 rounded-lg p3">Enviar</button>
        </form>
    </div>
    );
}